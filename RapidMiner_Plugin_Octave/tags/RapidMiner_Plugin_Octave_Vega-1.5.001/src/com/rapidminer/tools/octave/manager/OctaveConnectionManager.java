...
public class OctaveConnectionManager {
	...
	private OctaveConnectionManager() {
		...
		for (int i = 0; i < h.length; i++) {
			log.info("Adding log handler " + h[i].toString() + " to octave logger " + octlogger.getName());
...
//		e.close();
//		
//		OctaveEngineProxy p = new ThreadSafeSimpleOctaveEngineProxy(factory, "test");
//		p.shutdown();
//		System.out.println("closed engine successfully");
	...
	/**
	 * Method used by the various operators in order to get the UNIQUE octave
	 * engine. The engine is created and configured the first time it is called,
	 * then kept in memory for future use. In particular we configure it so that
	 ...
	 * @return
	 */
	public synchronized OctaveEngineProxy getScriptEngine(OctaveScriptOperator octaveScriptOperator) {
		if (enginePool == null) {
