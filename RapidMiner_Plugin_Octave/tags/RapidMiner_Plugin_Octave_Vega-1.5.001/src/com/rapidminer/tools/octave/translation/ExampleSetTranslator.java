...
public class ExampleSetTranslator implements OctaveTranslator<ExampleSet> {
	 ...
	 */
	@Override
	public ExampleSet importObject(OctaveEngineProxy octaveEngine, String objectName)
	 ...
	 * @throws UserError
	 */
	private ExampleSet importStruct(OctaveStruct dataExpStruct, String structName) throws UserError {
//		try{
		/* check fields of the structure: name, role, data, levelname */
		OctaveCell namecell = StructImportHelper.checkStructAttributeNames(dataExpStruct, structName);
		int nbAttributes = namecell.getSize()[1];
		OctaveCell rolecell = StructImportHelper.checkStructAttributeRoles(dataExpStruct, structName,
				nbAttributes);
		OctaveDouble dataMatrix = StructImportHelper.checkStructAttributesData(dataExpStruct, structName,
				nbAttributes);
		OctaveCell levelnameCell = StructImportHelper.checkStructNominalAttributeLevels(dataExpStruct,
				structName, nbAttributes);

		/* Extract the names and roles */
		String[] attributeNames = StructImportHelper.extractAttributeNames(namecell, structName);
		String[] attributeRoles = StructImportHelper.extractAttributeRoles(rolecell, structName);
		// int nbSpecialAttributes =
		// computeNbSpecialAttributes(attributeRoles);

		/* create Rapidminer attributes definitions */
		Attribute attributes[] = StructImportHelper.createRMAttributesDescriptionsForStructs(attributeNames,
				dataMatrix, levelnameCell);

		/* Save all data */
		MemoryExampleTable exampleTable = StructImportHelper.createRMExampleTable(attributes, dataMatrix);

		// now generate ExampleSet and declare its special attributes
		ExampleSet exampleSet = createRapidminerExampleSet(exampleTable, attributeRoles);

		return exampleSet;

//		 } catch (Exception e) {
//		 throw new UserError(null, e, "error.octave.data_translation_error.import.data.wrongsimportstruct",
//		 structName,
...
//		 }
	 ...
	 * @throws UserError
	 */
	private ExampleSet importCell(OctaveCell dataExpCell, String cellName) throws UserError {
//		 try {
		...
		/* create Rapidminer attributes definitions */
		Attribute attributes[] = c.createRMAttributeDescriptions(attributeNames);
...
//		 } catch (Exception e) {
//		 throw new UserError(null, e, "error.octave.data_translation_error.import.data.wrongsimportcell",
//		 cellName,
//		 ExampleSet.class.getSimpleName());
//		 }
	 ...
	 * @return
	 */
	private ExampleSet createRapidminerExampleSet(MemoryExampleTable exampleTable, String[] attributeRoles) {
		...
		while (itt.hasNext()) {
			...
			if (!(attributeRoles[numRole]).equals("")) {
				...
				clone.getAttributes().setSpecialAttribute(dd, attributeRoles[numRole]);
			}
			numRole++;
		}
		return clone;
	 ...
	 */
	@Override
	public void exportObject(OctaveEngineProxy octaveEngine, String expression, IOObject ioObject,
			String getParameter) throws OperatorException {
		...
		// Create a copy of Regular attributes
		int i = 0;
		...
		// Create a copy of Special attributes
		...
		while (it.hasNext()) {
			...
			// attributeNames[index] = attrole.getAttribute().getName();
			// copyAttributes[index] = attrole.getAttribute();
			//
			...
			// test = example.getValueAsString(attrole.getAttribute());
			...
			// }
			// NominalMapping nominalMapping = new PolynominalMapping(mapping);
			// // PolynominalAttribute pp = new PolynominalAttribute(test, j);
			//
			// attrole.getAttribute().setMapping(nominalMapping);
			//
			// attrole.getAttribute();
			//
			// } else {
			//
			// attributeNames[index] = attrole.getAttribute().getName();
		...
		for (Attribute attribute : allAttributesInclSpecial) {
			...
			} else if (/* date_time */Ontology.ATTRIBUTE_VALUE_TYPE.isA(attribute.getValueType(),
					Ontology.DATE_TIME)) {
...
//				int j = 0;
//				for (Example example : exampleSet) {
//					nominalDataValues[i][j] = example.getValueAsString(attribute);
//					j++;
//				}
			} 
			i++;
		...
		// copying data for each example in the set
		int exampleIndex = 0;
		for (Example example : exampleSet) {
			...
			for (Attribute attribute : allAttributesInclSpecial) {
				...
				if (attribute.isNumerical()) {
					...
					// for the struct encoding, we need to mix the encoded data with other data.
					numericalData[attributeIndex][exampleIndex] = levelEncoded + 1;

				} else if (/* date time */Ontology.ATTRIBUTE_VALUE_TYPE.isA(attribute.getValueType(),
						Ontology.DATE_TIME)) {
					
					// TODO check that DATE_TIME can be considered like a nominal.
					int levelEncoded = (int) value;
					encodedNominalData[attributeIndex][exampleIndex] = levelEncoded + 1;
					// for the struct encoding, we need to keep the encoded data.
					numericalData[attributeIndex][exampleIndex] = levelEncoded + 1;
		...
		if (getParameter.equals("cell")) {
			o = writeAsCell(allAttributesInclSpecial, attributes, numberOfExamples, numericalData,
					encodedNominalData);
		} else if (getParameter.equals("struct")) {
			o = writeAsStruct(allAttributesInclSpecial, attributes, numberOfExamples, numericalData);
		} else {
			throw new UserError(null, "error.octave.data_translation_error.import.data.wrongswrittendata",
					"Errors : the type of exemple set outputted is " + getParameter
	 ...
	 * @return
	 */
	private OctaveObject writeAsStruct(Attribute[] allAttributesInclSpecial, Attributes attributes,
			int numberOfExamples, double[][] numericalDataColumns) {
		...
		for (Attribute att : allAttributesInclSpecial) {
			...
			String role = attributes.findRoleByName(allAttributesInclSpecial[j].getName()).getSpecialName();
			cellRoles.set(new OctaveString(role == null ? "" : role), 1, j + 1);

			if (allAttributesInclSpecial[j].isNominal()) {

				int attributeNbLevels = attributes.get(allAttributesInclSpecial[j].getName()).getMapping()
						.getValues().size();
				OctaveCell attributeLevels = new OctaveCell(attributeNbLevels, 1);

				// copy all levels into a vertical cell, and put it in the
				// horizontal cell of levels
				for (int k = 0; k < attributeNbLevels; k++) {
					attributeLevels.set(new OctaveString(attributes
							.get(allAttributesInclSpecial[j].getName()).getMapping().getValues().get(k)),
							k + 1, 1);
				}
		...
		for (int n = 0; n < numberOfAttributesAll; n++) {
			System.arraycopy(numericalDataColumns[n], 0, allData, n * numberOfExamples, numberOfExamples);
		}
		...
		struct.put("data", new OctaveDouble(allData, numberOfExamples, numberOfAttributesAll));
	 ...
	 * @return
	 */
	private OctaveObject writeAsCell(Attribute[] allAttributesInclSpecial, Attributes attributes,
			int numberOfExamples, double[][] numericalData, int[][] nominalData) {
		...
		for (Attribute att : allAttributesInclSpecial) {
			...
			// role
			String role = attributes.findRoleByName(att.getName()).getSpecialName();
			...
			if (att.isNominal()) {
				...
				for (int k = 0; k < numberOfExamples; k++) {
					//TODO: since numericalData should contain the nominal data encoding, we could use numericalData in place of nominalData
					attributeLevels.set(
							new OctaveString(att.getMapping().getValues().get(nominalData[j][k]-1)), k + 1,
							1);
				}
				...
				OctaveDouble datatest = new OctaveDouble(numericalData[j], numberOfExamples, 1);
