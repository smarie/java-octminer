...
public class ExampleSetTranslator implements OctaveTranslator<ExampleSet> {
	 ...
	 */
	@Override
	public ExampleSet importObject(OctaveEngineProxy octaveEngine, String expression)
	 ...
	 * <li><b>data</b>: a 1x1 cell containing a m x n matrix of scalars where m is
		...
		try {
			...
			try {
				...
				for (int j = 0; j < nbCols; j++) {
					...
					// if there is , the number of numbrespecial + 1
			...
			/** for this new version, this Octave plugin can read all the type of role,
			* so we not use the method any more
			*/
...
//					weightName, idName, predictionName, clusterName, batchName,attributeRoles);
			...
			/** the new function for generate the ExampleSet with special attributes.
			*/
...
//	}
	 ...
	 * by checking the type in the levels object provided , the new parameter is 
	 * used to find the role values in levelname.
	 * @param name
	 * @param levels
	 * @param numcol
	 * @return
	 ...
	 * the new parameter is used to find the role values in levelname.
	 * @param name
	 * @param columnOctave
	 * @param numcol
	 * @return
	 ...
	 * Builds the Rapidminer attribute description by checking the type
	 * (numerical or "nominal", meaning an enumeration).
	 * This new method is created to find the level for the OctaveCell data,
	 * because the structure of OctaveStruct is changed , we can not use 
	 * the same method for the two type different, so we create the new.
	 * @param attName
	 * @param dataOrLevels
	 * @return
	 * @throws UserError
	...
	public void exportObject(OctaveEngineProxy octaveEngine, String expression,
		...
		/**
		 * if the special attribute is isNumerical();
		 * we add the index to the data matrix and change
		 * the value of attribute to String;
		 */
		Iterator<AttributeRole> itt = attributes.specialAttributes();
		int numRolet = numberOfAttributes ;
		...
		if (getParameter.equals("cell")) {
			OctaveObject oc;
			oc = writeAsCell(attributeNames, copyAttributes,exampleSet, attributes,
			...
			OctaveObject o;
			// if structure
			o = writeAsStruct(attributeNames, copyAttributes,exampleSet,attributes,
					numberOfExamples, numberOfAttributesAll, numericalData,
					nominalData);
	...
	private OctaveObject writeAsStruct(String[] attributeNames,
			Attribute[] copyAttributes,ExampleSet exampleSet, Attributes attributes,
			...
			if (role != null) {
				if (copyAttributes[j].isNumerical()){
					...
					for (Example example : exampleSet){
						...
						example.getValueAsString(attributes
						.findRoleByName(
...
//						.findRoleByName(
//								copyAttributes[j].getName())
//						.getAttribute()..getValues().get(nominalData[j][k]));
//					}
