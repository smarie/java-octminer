function CellData= MakeDataToCell(data,varargin)
	...
	elseif n==2 % name and role input 
		...
		for r=1:length(levelname)
			%Celltemps=[Celltemps;levelname{r,1}]; %bak levelname=Nx1 each element in an cell ,not in 1X1;
			%Celltemps(r,1) = levelname(r,1);
		end
			CellData{3,n}=levelname;
	end
end