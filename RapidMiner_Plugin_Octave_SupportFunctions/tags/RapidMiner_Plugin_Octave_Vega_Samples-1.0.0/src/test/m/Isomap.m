...
if (verbose > 0), disp('Computing shortest paths...'); end
...
if (verbose > 0),disp('Checking for outliers...'); end 
...
if (verbose > 0),disp(['  Number of connected components in graph: ' num2str(n_comps)]); 
disp(['  Embedding component ' num2str(comp) ' with ' num2str(size_comps(comp)) ' points.']); end
...
if (verbose > 0),disp('Constructing low-dimensional embeddings (Classical MDS)...'); end

opt.disp = 0; 
[vec, val] = eigs(-.5*(D.^2 - sum(D.^2)'*ones(1,N)/N - ones(N,1)*sum(D.^2)/N + sum(sum(D.^2))/(N^2)), 2, 'lm', opt); 
