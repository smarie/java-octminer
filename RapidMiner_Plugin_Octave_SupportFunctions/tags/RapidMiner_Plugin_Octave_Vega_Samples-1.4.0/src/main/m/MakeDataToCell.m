function CellData= MakeDataToCell(data,varargin)
	%%%%%%%%%%%%%%%%%
	%
	...
	elseif n==2 % name and role input 
		...
		CellData(2,:)=role;
		for i=1:nbDim
			if ~isempty(find(cellfun ( @isempty, (CellData(2,:)))== 1))
    			disp("Errors: the role values is not empty, please input the levelname! ")
    			CellData{3,i}=data(:,i);
			else
			CellData{3,i}=data(:,i);
			end
		...
		if ~isempty(find(cellfun ( @isempty, (CellData(2,:)))== 1))
			tmp=find(cellfun ( @isempty, (CellData(2,:)))== 0);
			
			for i=1:size(tmp,2)
				CellData{3,nbDim-size(tmp,2)+i}=levelname {nbDim-size(tmp,2)+i}(data(:,nbDim-size(tmp,2)+i)+1);	
			end
		end
