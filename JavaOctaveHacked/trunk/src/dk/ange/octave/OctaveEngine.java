...
///////////////////////////////////////
    public final OctaveExec octaveExec; // Hack - made public. Useful ?
    ...
    /**
     * New : timeouts for execution
     */
    public void setOctaveTimeouts(long writeTimeOut, long readTimeOut) {
        octaveExec.setOctaveTimeouts(writeTimeOut, readTimeOut);
    }
    ...
    public void eval(final String script) {
        ...
        // ---- TODO instead for getting the whole stack trace
        //unsafeEval(String.format("eval(javaoctave_%1$s_eval, \"javaoctave_%1$s_lasterr = lasterror();\");", tag));
        //final OctaveStruct[] lastError = get(OctaveStruct[].class, String.format("javaoctave_%1$s_lasterr", tag));
        //
        // however for that to work we need to modify StructReader.java to be able
        // to handle arrays of structs, with any number of fields.
        // ----
        ...
        if (lastError != null) {
        	...
        	// ---- TODO instead for getting the whole stack trace
        	 // this is a struct with fields "message" (string), "identifier", and "stack" (struct array)
            //OctaveString errMsg = lastError.get(OctaveString.class, "message");
            ...
            //			context = 0
            // e.stack(2)
            //throw new OctaveEvalException(errMsg.getString());
        	// ----
        }
    }
