...
public final class OctaveExec {
    ...
    // HACK sma : variables for timeout. Default value is Long.MAX_VALUE
    private long write_timeout_seconds = Long.MAX_VALUE;
    private long read_timeout_seconds = Long.MAX_VALUE;

    // HACK sma : made public so that we can change the start options of octave
    public static String[] CMD_ARRAY = { null, "--no-history", "--no-init-file", "--no-line-editing",
            "--no-site-file", "--silent" };
     ...
     */
    public synchronized void setOctaveTimeouts(long writeTimeOut,
            long readTimeOut) {
    ...
    public void eval(final WriteFunctor input, final ReadFunctor output) {
        ...
        final RuntimeException writerException = getFromFuture(writerFuture,
                write_timeout_seconds, "Sending message to octave engine");
        ...
        final RuntimeException readerException = getFromFuture(readerFuture,
                read_timeout_seconds, "Reading response from octave engine");
        if (writerException != null) {
    ...
    private RuntimeException getFromFuture(final Future<Void> future,
            long timeoutInSeconds, String operationName) {
        try {
            ...
            final String message = "TimeoutException : " + operationName
                    + " took more than " + timeoutInSeconds
                    + " seconds";
            log.error(message, e);
            return new RuntimeException(message, e);
        }
